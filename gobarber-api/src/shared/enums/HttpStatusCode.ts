...
enum HttpStatusCode {
   ...
   * send the request body (in the case of a request for which a body needs to
   * be sent; for example, a POST request). Sending a large request body to a
   * server after a request has been rejected for inappropriate headers would
   * be inefficient. To have a server check the request's headers, a client must
   ...
   * is no need to retransmit the resource since the client still has a previously-downloaded copy.
   */
  NOT_MODIFIED = 304,
   ...
   * should be purged. Upon receiving a 410 status code, the client should not request
   * the resource in the future. Clients such as search engines should remove the
   * resource from their indices. Most use cases do not require clients and search
   ...
   * too much data being encoded as a query-string of a GET request,
   ...
   * in RFC 2324, Hyper Text Coffee Pot Control Protocol, and is not expected to
   * be implemented by actual HTTP servers. The RFC specifies this code should be returned by
   * teapots requested to brew coffee. This HTTP status is used as an Easter egg in
   * some websites, including Google.com.
   */
  I_AM_A_TEAPOT = 418,
   ...
   * to fulfill the request. Usually this implies future availability (e.g., a new
